...
def fetch_foreign_flow_and_non_regular_ticker_data(active_market_dates: list, with_docker: bool) -> list:
    """
    ...
    driver = _initialize_driver(with_docker)
    ...
    for active_market_date in active_market_dates:
        try:
            ...
            process_data_bool = _select_and_download_specific_date_on_web(driver, active_market_date)
            
            if process_data_bool:
                _clean_downloaded_data(active_market_date)
...
def process_foreign_flow_and_non_regular_ticker_data(args_tuple) -> None:
    ...
    Args:
        args_tuple (tuple): A Tuple containing (ticker, combined_df)
    """
    ticker, combined_df = args_tuple
    ...
    save_path = paths.get_foreign_flow_non_regular_path(ticker)

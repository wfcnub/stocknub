...
def _ensure_directories_exist(label_types):
    ...
    for label_type in label_types:
        ...
        Path(f"data/stock/model_v2/{camel_label}").mkdir(
            parents=True, exist_ok=True
        )
        Path(f"data/stock/model_v2/performance/{camel_label}").mkdir(
            parents=True, exist_ok=True
        )
    
    return

def _save_model(model, label_type, industry, window):
    """
    (Internal Helper) Save a trained model to file.
    """
    camel_label = to_camel(label_type)
    filepath = f"data/stock/model_v2/{camel_label}/{industry}-{window}dd.pkl"
...
def _combine_metrics(train_metrics, test_metrics):
    """
